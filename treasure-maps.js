...
(function () {
    ...
    // --------------------------------------------------------------------------
    // Xrel.to-Handler
    // Release-Namen → Textsuche ohne Kategorie (cat=-1)
    // Icon erscheint in Release-Listen, in der NFO-Ansicht und auf Extinfo-Seiten
    // Hinweis: Release-Namen werden unverändert übernommen (kein cleanTitle nötig)
    // --------------------------------------------------------------------------
    function xrel_insertIcon(container, title, refNode) {
        if (!container || !title || !refNode) return;

        // Prüfen, ob bereits ein Icon existiert → doppelte Icons vermeiden
        if (container.querySelector('a[title^="Suche auf treasure-maps"]')) return;
    ...
    function handleXrel() {
        if (!SHOW_ICON_ON.Xrel) return;
        if (!location.hostname.includes('xrel.to')) return;

        // A) Release-Listen (Scene und P2P)
        document.querySelectorAll('.release_item, .release_item_p2p').forEach(item => {
            const titleDiv = item.querySelector('.release_title, .release_title_p2p');
            if (!titleDiv) return;

            // Release-Name aus dem title-Attribut des gekürzten Links,
            // sonst Fallback auf den letzten Link im Titel-Div
            let title = titleDiv.querySelector('.dirname-truncated a')?.title?.trim();
            if (!title) {
                const links = titleDiv.querySelectorAll('a');
                const last = links[links.length - 1];
                title = (last?.title || last?.textContent || '').trim();
            }

            const firstLink = titleDiv.querySelector('a');
            if (firstLink) xrel_insertIcon(titleDiv, title, firstLink);
        });

        // B) NFO-Ansicht: Release-Name im dirname-Span
        const nfoTitleDiv = document.querySelector('.nfo_title');
        if (nfoTitleDiv) {
            const dirnameSpan = nfoTitleDiv.querySelector('#nfo-view-dirname');
            if (dirnameSpan) {
                xrel_insertIcon(nfoTitleDiv, dirnameSpan.textContent.trim(), dirnameSpan);
            }

            // C) NFO-Ansicht, alternative Struktur: Link im .sub-Bereich
            const subLink = nfoTitleDiv.querySelector('.sub a');
            if (subLink) {
                xrel_insertIcon(nfoTitleDiv, subLink.textContent.trim(), subLink);
            }
        }

        // D) Extinfo-Seiten: Titel im <h3>
        const extinfoTitleDiv = document.querySelector('#extinfo_title');
        if (extinfoTitleDiv) {
            const h3 = extinfoTitleDiv.querySelector('h3');
            if (h3 && h3.lastChild) {
                xrel_insertIcon(extinfoTitleDiv, h3.textContent.trim(), h3.lastChild);
            }
        }
    }
    ...
    function init() {
        ...
        else if (host.includes('xrel.to')) handleXrel();
    }
